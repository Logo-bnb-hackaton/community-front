...
const ProfileEdit: React.FC<Props> = ({
    ...
    const logoDraggerHandler = (base64Logo: string | undefined) => {
        setProfile({...profile, logo: {id: undefined, base64Image: base64Logo}});
    };
    ...
    const descriptionInputHandler = (description: string) => {
        setProfile({...profile, description: description.slice(0, MAX_DESCRIPTION_LEN)});
    };
    ...
    const enableOrDisableToken = async (tokenSymbol: string) => {
        ...
        try {
            ...
            setAvailableTokens(
                await Contract.profile.loadAvailableTokens(id)
            );
        } catch (e) {
            console.error(e);
        } finally {
    ...
    return (
        ...
        >
            ...
            >
                <h2 style={{marginBottom: '48px'}}>Please, choose tokens for donation:</h2>
