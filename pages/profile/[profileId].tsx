...
const Profile: NextPage<Props> = ({profile, ownerId, tokens}) => {
    ...
    useEffect(() => {
        ...
        setSocialMediaLinks(profile ? profile.socialMediaLinks.map(link => toSocialMediaLink(link)) : []);
    }, [profile]);
    ...
    const enableOrDisableToken = async (tokenSymbol: string) => {
        ...
        try {
            ...
            await Contract.profile.enableOrDisableToken(profileId as string, tokenAddress, !enabled);
            setAvailableTokens(await Contract.profile.loadAvailableTokens(profileId as string));
        } catch (e) {
            console.error(e);
        } finally {
            setIsLoading(false);
    ...
    const getAvailableSubscriptions = () => {
        if (!profile) return [];
        return profile.subscriptions.filter(s => editAvailable || s.status !== 'DRAFT')
    }
            ...
            <div className={styles.center}
                ...
                {editAvailable && !edited &&
                    <Button
                        ...
                        onClick={() => router.push(`/subscription/create?profileId=${profileId}`)}
